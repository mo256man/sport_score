...
def get_handballdata():
    url = "https://leagueh.jp/rankings/matrix/"
    driver = webdriver.Edge(service=Service(r"./msedgedriver.exe"))
    driver.get(url)
    match_links = []
    table = driver.find_element(By.CLASS_NAME, "matrix-table")
    ...
    match_links = list(set(match_links))        # 重複を排除
    ...
    for url in match_links:
        ...
        data = row.text.split("\n")
        print(data)
        # ['ブレイヴキングス刈谷', '20240921（土）14:30', '枇杷島スポーツセンター', '35', '23', '18', '前半', '10', '17', '後半', '13', 'レッドトルネード佐賀']
        pass
